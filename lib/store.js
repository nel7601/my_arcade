...
// Tolerate common paste accidents in the env var: surrounding quotes,
// a leading "psql ", or stray whitespace
function cleanUrl(raw) {
  return String(raw || '').trim().replace(/^psql\s+/i, '').replace(/^['"]|['"]$/g, '').trim();
}

const PG_URL = cleanUrl(process.env.DATABASE_URL || process.env.POSTGRES_URL || process.env.NEON_DATABASE_URL);
