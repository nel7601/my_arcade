...
import { incr, set } from './store.js';

// Best-effort stat counters: never let stats break the game
const count = (key) => { incr(key).catch(() => {}); };

// Played counter + last-played timestamp (epoch ms, so it rides along
// with the numeric counters in the back-office stats query)
const markPlayed = (game) => {
  const g = game || 'unknown';
  count('stat:played:' + g);
  set('stat:lastplay:' + g, String(Date.now())).catch(() => {});
};
  ...
  ws.on('message', (raw) => {
    ...
    switch (msg.type) {
      case 'create': {
        ...
        // `game` tags the room; `opts` are small game-specific numbers.
        // seconds 0 = endless match (no clock); otherwise clamp to 10..600
        const rawSecs = Math.round(Number(msg.seconds));
        const seconds = rawSecs === 0 ? 0 : Math.min(600, Math.max(10, rawSecs || 240));
        const game = String(msg.game || '').slice(0, 20);
        const opts = {};
        if (msg.opts && typeof msg.opts === 'object') {
          for (const key of Object.keys(msg.opts).slice(0, 8)) {
            const v = Number(msg.opts[key]);
            if (Number.isFinite(v)) {
              opts[String(key).slice(0, 16)] = Math.max(-999, Math.min(999, Math.round(v)));
            }
          }
        }
        const room = {
        ...
        rooms.set(code, room);
        attach(ws, room, 'host');
        count('stat:created:' + (game || 'unknown'));
      ...
      case 'join': {
        ...
        // A snake link must not land you in a pong room
        if (room.config.game && msg.game && room.config.game !== msg.game) {
          return slotSend({ ws, queue: [] }, { type: 'error', reason: 'not_found' });
        }
        if (room.guest) {
      ...
      default: {
        ...
        if (msg.type === 'goal') {
          ...
          // Self-scoring games: the sender reports its own new total
          room.score[ws.pongRole] = Number(msg.mine) || 0;
          room.rematchVotes.clear();
